function data = CEAMatrix(filename)
...
% associated with that OF ratio value: 2 column is R; 3 column is Pc; 4
% column is Pt; 5 column is P_exit_sub; 6 column is P_exit_sup; 7  column 
% is Tc; 8 columns is Tt; 9 column is cp_chamber; 10 column is cp_throat; 11 column is
% gamma_c; 12 column is M_exit_sub; 13 column is M_exit_sup; 14 column is
% cstar_sub; 15 column is cstar_sup; 16 column is CF_sub; 17 column is
% CF_sup; 18 column is Ivac_sub; 19 column is Ivac_sup; 20 column is
% Isp_sub; 21 column is Isp_sup.
...
while ischar(tline)
    ...
    if contains(tline, 'THEORETICAL ROCKET PERFORMANCE ASSUMING FROZEN COMPOSITION')
        ...
        % Extract O/F value and R
        of_value_parts = regexp(tline, 'O/F=(\s*\d+\.?\d*)', 'tokens');
        R_value_parts = regexp(tline, 'R,EQ.RATIO=(\s*\d+\.?\d*)', 'tokens');
        ...
        if ~isempty(R_value_parts)
            R_value_str = R_value_parts{1}{1};
            R_value = str2double(R_value_str);
        ...
        data_cur = [P_cur, T_cur, cp_cur, gamma_cur, M_cur];
...
% plotting
data = [of_values R_values data_values performance];
end